...
export default function AddBookingModal({ 
  ...
  totalBaseAmount, 
  balanceDue,
  isRoomFixed = false // New prop to control whether the room field is locked
}) {
  if (!showModal) return null;
  ...
  // Check if any required field is completely blank/empty
  const isRequiredMissing = !form.name?.trim() || !form.contact?.trim() || !form.checkIn || !form.checkOut || !form.room;
          ...
          <form onSubmit={executeSave} className="p-6 space-y-6 h-auto">
                ...
                <div>
                  ...
                  {isRoomFixed ? (
                    <input 
                      ...
                      type="text" 
                      value={form.room || ''} 
                      className="w-full border border-gray-200 bg-gray-100 rounded-lg p-2 text-sm mt-1 font-bold text-gray-700 cursor-not-allowed outline-none" 
                    />
                  ) : (
                    <select 
                      value={form.room || ''} 
                      onChange={(e) => handleInputChange('room', e.target.value)} 
                      className="w-full border rounded-lg p-2 text-sm mt-1 bg-white outline-none focus:border-[#7A1C31]"
                    >
              ...
              <div className="grid grid-cols-4 gap-4">
                <div>
                  ...
                  <input 
                    ...
                    onChange={(e) => handleInputChange('rate', e.target.value === '' ? '' : parseFloat(e.target.value))} 
                    className={`w-full border rounded-lg p-2 text-sm mt-1 outline-none transition-colors ${
                      isRateInvalid ? 'border-rose-500 bg-rose-50/50 text-rose-900 focus:border-rose-600' : 'border-gray-200 focus:border-[#7A1C31]'
                    }`}
                    placeholder="0" 
                  />
                  {isRateInvalid && <p className="text-[11px] font-medium text-rose-600 mt-1.5 animate-pulse">⚠️ Rate cannot be negative.</p>}
                </div>
                ...
                <div>
                  ...
                  <input 
                    ...
                    onChange={(e) => handleInputChange('extra', e.target.value === '' ? '' : parseFloat(e.target.value))} 
                    className={`w-full border rounded-lg p-2 text-sm mt-1 outline-none transition-colors ${
                      isExtraInvalid ? 'border-rose-500 bg-rose-50/50 text-rose-900 focus:border-rose-600' : 'border-gray-200 focus:border-[#7A1C31]'
                    }`}
                    placeholder="0" 
                  />
                  {isExtraInvalid && <p className="text-[11px] font-medium text-rose-600 mt-1.5 animate-pulse">⚠️ Charges cannot be negative.</p>}
                </div>
                ...
                <div>
                  ...
                  <input 
                    ...
                    onChange={(e) => handleInputChange('downpayment', e.target.value === '' ? '' : parseFloat(e.target.value))} 
                    className={`w-full border rounded-lg p-2 text-sm mt-1 outline-none transition-colors ${
                      isDownpaymentInvalid ? 'border-rose-500 bg-rose-50/50 text-rose-900 focus:border-rose-600' : 'border-gray-200 focus:border-[#7A1C31]'
                    }`}
                    placeholder="0" 
                  />
                  {isDownpaymentInvalid && <p className="text-[11px] font-medium text-rose-600 mt-1.5 animate-pulse">⚠️ Cannot be negative.</p>}
                </div>

                <div>
                  <label className="text-xs font-bold text-gray-600">Payment Status</label>
                  <select value={form.paymentStatus || 'Unpaid'} onChange={(e) => handleInputChange('paymentStatus', e.target.value)} className="w-full border rounded-lg p-2 text-sm mt-1 outline-none focus:border-[#7A1C31]">
                    <option>Unpaid</option>
                    <option>Partial</option>
                    <option>Fully Paid</option>
                  </select>
            ...
            <div className="flex justify-end gap-2 pt-4 border-t">
              ...
              <button 
                ...
                disabled={isSubmitDisabled}
                className={`px-5 py-2 text-white rounded-lg text-sm font-medium transition-all duration-200 ${
                  isSubmitDisabled ? 'bg-gray-300 cursor-not-allowed opacity-60' : 'bg-[#7A1C31] hover:bg-[#631425] shadow-sm'
                }`}
              >
                Save Booking
              </button>
